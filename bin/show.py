...
for i in range(len(GPARAMS.MD_setting)):
    ...
    for j in range(GPARAMS.Train_setting.Trainstage):
        ...
        moddevlayout=go.Layout(
            autosize=True,
            xaxis={"title":"Time (ps)"},
            yaxis={"title":"Error Indicator (kcal/mol/Å)"},
...
app.layout=html.Div(children=[
    ...
    [
        html.H5(children="------- Structure Optimization of Meta-networks with Genetic Algorithm -------\n",style={"margin":"auto","text-align":"center"}),
        html.Div(children=generate_table(df),className='pretty_container twelve columns'),
        html.H5(children="------- MD Simulation in E-SOI-HDNN Training Process -------",\
                style={"margin":"auto","text-align":"center"})
    ]+\
    [
        html.Div(children=[
