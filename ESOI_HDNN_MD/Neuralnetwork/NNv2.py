    ...
    def __init__(self, TData_, Name_=None, Trainable_=True,ForceType_="LJ",Structure=None,LR=None):
        ...
        if Name_!=None:
            ...
            if self.TData!=None and TData_== None and Trainable_==True:
                ...
                self.learning_rate_charge =self.learning_rate
                ...
                self.LR_boundary_q=[math.ceil(m) for m in np.array(GPARAMS.Neuralnetwork_setting.Learningrateboundary)\
                                     *GPARAMS.Train_setting.Maxsteps*(1-GPARAMS.Neuralnetwork_setting.Switchrate[1])]#+self.switch_steps]
        ...
        self.LR_boundary=[math.ceil(m) for m in np.array(GPARAMS.Neuralnetwork_setting.Learningrateboundary)*GPARAMS.Train_setting.Maxsteps]
        self.LR_boundary_dipole=[math.ceil(m) for m in np.array(GPARAMS.Neuralnetwork_setting.Learningrateboundary)\
                         *GPARAMS.Train_setting.Maxsteps*GPARAMS.Neuralnetwork_setting.Switchrate[0]]
        self.LR_boundary_ef=[math.ceil(m) for m in np.array(GPARAMS.Neuralnetwork_setting.Learningrateboundary)\
                     *GPARAMS.Train_setting.Maxsteps*(GPARAMS.Neuralnetwork_setting.Switchrate[1]-GPARAMS.Neuralnetwork_setting.Switchrate[0])]#+self.switch_steps]
        self.LR_boundary_q=[math.ceil(m) for m in np.array(GPARAMS.Neuralnetwork_setting.Learningrateboundary)\
                     *GPARAMS.Train_setting.Maxsteps*(1-GPARAMS.Neuralnetwork_setting.Switchrate[1])]#+self.switch_steps]
        ...
        self.ChargeScalar=GPARAMS.Neuralnetwork_setting.Scalar["Q"]
...
#            with tf.name_scope("behler"):
            self.Etotal, self.Ebp, self.Evdw,  self.energy_wb, self.Ebp_atom = \
                self.energy_inference(self.Scatter_Sym, self.Sym_Index, self.Ecc, self.xyzs_pl, self.Zs_pl,\
                    Ele, C6, vdw_R, self.Reep_pl, Ree_on, Ree_off, self.keep_prob_pl,self.batch_size_ctrl)
            self.resp_charge, self.charge_wb = \
                self.charge_inference(self.Scatter_Sym, self.Sym_Index, self.xyzs_pl, self.natom_pl,\
...
#            with tf.name_scope("losses"):
            ...
            tf.summary.scalar("loss_charge",self.loss_charge)
#            with tf.name_scope("training"):
            ...
            self.train_op_charge,self.lr_charge = self.training(self.total_loss_charge, self.learning_rate_charge,self.LR_boundary_charge, self.momentum, self.charge_wb)
    ...
    def dipole_inference(self, inp, indexs, xyzs, natom, Elu_Width, EE_cutoff, Reep, AddEcc, keep_prob,batch_size_ctrl):
        ...
        with tf.name_scope("DipoleNet"):
            for e in range(len(self.eles)):
                ...
                for i in range(len(self.HiddenLayers)):
                    if i == 0:
                        with tf.name_scope(str(self.eles[e])+'_hidden1_dipole'):
                            weights = self._variable_with_weight_decay(var_name='weights', var_shape=[self.inshape, self.HiddenLayers[i]], var_stddev=1.0/(10+math.sqrt(float(self.inshape))), var_wd=0.001)
                        ...
                        with tf.name_scope(str(self.eles[e])+'_hidden'+str(i+1)+"_dipole"):
                            weights = self._variable_with_weight_decay(var_name='weights', var_shape=[self.HiddenLayers[i-1], self.HiddenLayers[i]], var_stddev=1.0/(10+math.sqrt(float(self.HiddenLayers[i-1]))), var_wd=0.001)
                            biases = tf.Variable(tf.zeros([self.HiddenLayers[i]], dtype=self.tf_prec), name='biases')
                            ...
                            dipole_wb.append(weights)
                            dipole_wb.append(biases)
                with tf.name_scope(str(self.eles[e])+'_regression_linear_dipole'):
        ...
        def dipf1(): return TFCoulombEluSRDSFLR(xyzsInBohr, scaled_charge, Elu_Width*BOHRPERA, Reep, tf.cast(self.DSFAlpha, self.tf_prec), tf.cast(self.elu_alpha,self.tf_prec), tf.cast(self.elu_shift,self.tf_prec))
        #IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII
        #def f2(): return  tf.zeros([self.batch_size], dtype=self.tf_prec)
        def dipf2(): return  tf.zeros(batch_size_ctrl, dtype=self.tf_prec)
        #IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII
        cc_energy = tf.cond(AddEcc, dipf1, dipf2)
                ...
                for i in range(len(self.HiddenLayers)):
                    if i == 0:
                        with tf.name_scope(str(self.eles[e])+'_hidden1_charge'):
                            ...
                            biases = tf.Variable(tf.zeros([self.HiddenLayers[i]], dtype=self.tf_prec), name='biases')
                            Dbranches[-1].append(self.activation_function(tf.matmul(tf.nn.dropout(charge_inputs, keep_prob[i]), weights) + biases))
                            charge_wb.append(weights)
                            charge_wb.append(biases)
                    else:
...
#           dipole = tf.reduce_sum(tf.reshape(flat_dipole,tf.concat([batch_size_ctrl,[self.MaxNAtoms,3]],axis=0)), axis=1)
        #def chargef1(): return TFCoulombEluSRDSFLR(xyzsInBohr, final_charge, Elu_Width*BOHRPERA, Reep, tf.cast(self.DSFAlpha, self.tf_prec), tf.cast(self.elu_alpha,self.tf_prec), tf.cast(self.elu_shift,self.tf_prec))
        #def chargef2(): return  tf.zeros(batch_size_ctrl, dtype=self.tf_prec)
        #cc_energy = tf.cond(AddEcc, chargef1, chargef2)
        return   final_charge, charge_wb
    ...
    def loss_op_charge(self, energy_grads, charge ,Qlabels ,natom):
        maxatom=tf.cast(tf.shape(energy_grads)[2], self.tf_prec)
    ...
    def train_step_charge (self,step):
        Ncase_train       = self.TData.NTrain
        start_time        = time.time()
        train_loss        = 0.0
        ...
        for ministep in range (0, int(Ncase_train/self.batch_size)):
            ...
            actual_mols   = self.batch_size
            t             = time.time()
            dump_2, total_loss_value,  loss_value,charge_loss,lr_charge \
                = self.sess.run([self.train_op_dipole, self.total_loss_charge, self.loss_charge, self.charge_loss_charge,self.lr_dipole],\
            ...
            duration           = time.time() - start_time
            num_of_mols       += actual_mols
        self.print_training_charge(step, train_charge_loss, num_of_mols, duration,lr_dipole)
    ...
    def test_charge(self, step):
        ...
        for ministep in range (0, int(Ncase_test/self.batch_size)):
            ...
            actual_mols  = self.batch_size
            t            = time.time()
            total_loss_value, loss_value,  charge_loss =\
                 self.sess.run([self.total_loss_charge, self.loss_charge, self.charge_loss_charge], feed_dict=self.fill_feed_dict(batch_data))
            print (' mini step:')
            test_loss         = test_loss + loss_value
            ...
            num_of_mols      += actual_mols
        print ("testing...")
        self.print_training_charge(step,  test_charge_loss, num_of_mols, duration,0,False)
        return  test_loss

    def train(self, mxsteps, continue_training= False,chk_file='',AimE=0.1,AimF=0.002,AimD=0.005,AimQ=0.005):
        """
...
#        for step in  range (0, mxsteps):
        ...
        while ifcontinue :
            if self.Training_Target == "EandG":
                ...
                if step%test_freq==0 and step!=0 :
                    ...
                    if test_energy_loss < mini_energy_test_loss:
                        ...
                        self.save_chk(step)
                if (Lossf<=AimF and Losse<=AimE) or (step>=math.ceil(maxsteps*GPARAMS.Neuralnetwork_setting.Switchrate[1])):
                    self.recorder.write("Training of %s stop due to achieve Force aim or beyond max training steps "%self.name)
                     #ifcontinue=False 
                    self.Training_Target="Charge"
                    self.recorder.write("Switching to Charge Training...\n")
                ...
                if step%test_freq==0 and step!=0 :
                    ...
                    if test_dipole_loss < mini_dipole_test_loss:
                        ...
                        self.save_chk(step)
                    if Lossd<=AimD or step>=math.ceil(maxsteps*GPARAMS.Neuralnetwork_setting.Switchrate[0]):
                ...
                if step%test_freq==0 and step!=0 :
                    if self.monitor_mset != None:
                        self.InTrainEval(self.monitor_mset, self.Rr_cut, self.Ra_cut, self.Ree_off, step=step)
                    test_charge_loss = self.test_charge(step)
                    if test_charge_loss < mini_charge_test_loss:
                        mini_charge_test_loss = test_charge_loss
                        self.save_chk(step)
                    if Lossd<=AimD or step>=math.ceil(maxsteps*GPARAMS.Neuralnetwork_setting.Switchrate[0]):
                        self.recorder.write("Training of %s switch to E&G due to achieve Dipole aim or beyond max training steps %d\n"%(self.name,self.max_steps*GPARAMS.Neuralnetwork_setting.Switchrate))
                        self.saver.restore(self.sess, self.chk_file)
            ...
            step+=1
        self.SaveAndClose()
        return  self.TData.NTrain,self.batchtrainnumf,Losse,Lossf,self.batchtrainnumd,Lossd,self.batchtrainbumq,Lossq,self.HiddenLayers
    ...
    def InTrainEval(self, mol_set, Rr_cut, Ra_cut, Ree_cut, step=0):
        ...
        Etotal, Ebp, Ebp_atom, Ecc, Evdw, mol_dipole, resp_charge, gradient = self.sess.run([self.Etotal, self.Ebp, self.Ebp_atom, self.Ecc, self.Evdw, self.dipole, self.resp_charge, self.gradient], feed_dict=feed_dict)
        ...
        return Etotal, Ebp, Ebp_atom, Ecc, Evdw, mol_dipole, resp_charge, gradient
