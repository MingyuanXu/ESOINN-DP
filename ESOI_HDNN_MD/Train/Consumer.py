...
def consumer(Queue):
    ...
    input_path='./'+GPARAMS.Compute_setting.Theroylevel+'/Consumer'
    if not os.path.exists(input_path):
        os.system("mkdir "+input_path)
    Trainingset=MSet(GPARAMS.Compute_setting.Theroylevel)
    backupname="%s_backup"%GPARAMS.Compute_setting.Theroylevel  
    if os.path.exists('./datasets/%s'%GPARAMS.Compute_setting.Theroylevel):
        Trainingset.Load()
    ...
    while True:
        ...
        for i in range(len(ERROR_mols)):
            if 'energy' not in ERROR_mols[i].properties.keys():
                ...
                elif GPARAMS.Compute_setting.Theroylevel=="":
                    pass
