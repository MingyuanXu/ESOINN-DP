...
class BP_HDNN_charge():
    ...
    def Clean(self):
        ...
        self.gradient = None
        self.total_loss_dipole, self.loss_dipole, self.energy_loss_dipole, self.grads_loss_dipole, self.dipole_loss_dipole,self.charge_loss_dipole = None, None, None, None, None,None
        ...
        self.lr_dipole,self.lr_ef=None,None
    ...
    def train(self, mxsteps, continue_training= False,chk_file='',AimE=0.05,AimF=0.005,AimD=0.05):
        """
...
#        for step in  range (0, mxsteps):
        ...
        while ifcontinue :
            if self.Training_Target == "EandG":
                ...
                if step%test_freq==0 and step!=0 :
                    ...
                    if test_grads_loss < mini_grads_test_loss:
                        mini_grads_test_loss = test_grads_loss 
                        self.save_chk(step)
                ...
                Lossq=self.train_step_dipole(step)
                ...
                if step%test_freq==0 and step!=0 :
                    ...
                    if test_dipole_loss < mini_dipole_test_loss:
                        ...
                        self.save_chk(step)
                    if Lossq<=AimD or step>=math.ceil(maxsteps*GPARAMS.Neuralnetwork_setting.Switchrate):
                    ...
                    #if len(recordlossq)>20:
                    #    self.recorder.write("Avg Lossd in last 5-10 steps: %f , Lossd in lass 1-5 steps: %f judge value: %f\n"\
                    #                         %(np.mean(recordlossq[-20:-10]),np.mean(recordlossq[-10:-1]),(recordlossq[-1]-AimD)/50))
                    #    if np.mean(recordlossq[-10:5])-np.mean(recordlossq[-5:-1])<(recordlossq[-1]-Aimf)/50:
                    #        self.saver.restore(self.sess, self.chk_file)
                    #        self.Training_Target = "EandG"
                    #        self.recorder.write("Switching to Energy and Gradient Learning...\n")
            step+=1
        self.SaveAndClose()
        return  self.TData.NTrain,self.batchtrainnumf,Losse,Lossf,self.batchtrainnumd,Lossq,self.HiddenLayers
    ...
    def TrainPrepare(self,  continue_training =False,chk_file=''):
        ...
        with self.graph.as_default():
            ...
            self.Qtlabel_pl = tf.placeholder(self.tf_prec, shape=tuple([None]),name="DesCharge_total")
            self.masks_pl = tf.placeholder(self.tf_prec, shape=tuple([None, self.MaxNAtoms]),name="Desmasks")
            ...
            self.Scatter_Sym, self.Sym_Index  = \
                TFSymSet_Scattered_Linear_WithEle(self.xyzs_pl, self.Zs_pl, Ele, SFPr2, Rr_cut, Elep, SFPa2, zeta,\
                                                     eta, Ra_cut, self.Radp_Ele_pl, self.Angt_Elep_pl, self.mil_jk_pl)
            self.Ecc,self.dipole, self.charge, self.dipole_wb = \
                self.dipole_inference(self.Scatter_Sym, self.Sym_Index, self.xyzs_pl, self.natom_pl,\
                self.masks_pl,self.Qtlabel_pl, elu_width, Ree_off, self.Reep_pl, self.AddEcc_pl, self.keep_prob_pl,self.batch_size_ctrl)
            self.Radp_pl  = self.Radp_Ele_pl[:,:3]
            self.Etotal, self.Ebp, self.Evdw,  self.energy_wb, self.Ebp_atom = \
                self.energy_inference(self.Scatter_Sym, self.Sym_Index, self.Ecc, self.xyzs_pl,\
                 self.Zs_pl, Ele, C6, vdw_R, self.Reep_pl, Ree_on, Ree_off, self.keep_prob_pl,self.batch_size_ctrl)

            self.gradient  = tf.gradients(self.Etotal, self.xyzs_pl, name="BPEGrad")

            self.total_loss_dipole, self.loss_dipole,self.dipole_loss_dipole,self.charge_loss_dipole\
                 = self.loss_op_dipole(self.Etotal,self.gradient,self.dipole,self.charge,\
                     self.Dlabel_pl, self.Qlabel_pl, self.natom_pl)

            self.total_loss_EandG, self.loss_EandG, self.energy_loss_EandG, self.grads_loss_EandG = \
                 self.loss_op_EandG(self.Etotal, self.gradient, self.Elabel_pl, self.grads_pl,self.natom_pl)
    ...
    def fill_feed_dict(self, batch_data):
        ...
        feed_dict={i: d for i, d in zip([self.xyzs_pl]+[self.Zs_pl]+[self.Elabel_pl]+[self.Dlabel_pl]+[self.Qlabel_pl]+[self.Qtlabel_pl]+[self.masks_pl]+[self.grads_pl] + [self.Radp_Ele_pl] + [self.Angt_Elep_pl] + [self.Reep_pl] + [self.mil_jk_pl] + [self.natom_pl] + [self.AddEcc_pl] + [self.keep_prob_pl], batch_data)}

        feed_dict[self.batch_size_ctrl]=[batch_data[2].shape[0]]

        return feed_dict

    def dipole_inference(self, inp, indexs, xyzs, natom, masks, total_c, Elu_Width, EE_cutoff, Reep, AddEcc, keep_prob,batch_size_ctrl):
        xyzsInBohr = tf.multiply(xyzs,BOHRPERA)
        Dbranches=[]
        atom_outputs_charge = []
        #self.ele_list_for_check=[]
        ...
        with tf.name_scope("DipoleNet"):
            for e in range(len(self.eles)):
                #self.ele_list_for_check.append([])
                Dbranches.append([])
                charge_inputs = inp[e]
                    ...
                    if i == 0:
                        ...
                        with tf.name_scope(str(self.eles[e])+'_hidden'+str(i+1)+"_charge"):
                            weights = self._variable_with_weight_decay(var_name='weights',\
                                 var_shape=[self.HiddenLayers[i-1], self.HiddenLayers[i]], var_stddev=1.0/(10+math.sqrt(float(self.HiddenLayers[i-1]))), var_wd=0.001)
                            biases = tf.Variable(tf.zeros([self.HiddenLayers[i]], dtype=self.tf_prec), name='biases')
                ...
                with tf.name_scope(str(self.eles[e])+'_regression_linear_charge'):
                    ...
                    #self.ele_list_for_check[e].append([Dbranches[-1][-1],shp_out,cut,rshp,rshpflat,atom_indice,ToAdd,output_charge])
            tf.verify_tensor_all_finite(output_charge,"Nan in output!!!")
            netcharge = total_c-tf.reshape(tf.reduce_sum(output_charge, axis=1),[-1])
            delta_charge = tf.multiply(netcharge, natom)
            delta_charge_tile = tf.tile(tf.reshape(delta_charge,[-1,1]),[1, self.MaxNAtoms])
            scaled_charge =  tf.add(output_charge, delta_charge_tile)
            final_charge  =  tf.multiply(scaled_charge, masks)
            flat_dipole = tf.multiply(tf.reshape(xyzsInBohr,tf.concat([batch_size_ctrl*self.MaxNAtoms,[3]],axis=0)), tf.reshape(final_charge,tf.concat([batch_size_ctrl*self.MaxNAtoms,[1]],axis=0)))
            dipole = tf.reduce_sum(tf.reshape(flat_dipole,tf.concat([batch_size_ctrl,[self.MaxNAtoms,3]],axis=0)), axis=1)
             
        def f1(): return TFCoulombEluSRDSFLR(xyzsInBohr, final_charge, Elu_Width*BOHRPERA, Reep, tf.cast(self.DSFAlpha, self.tf_prec), tf.cast(self.elu_alpha,self.tf_prec), tf.cast(self.elu_shift,self.tf_prec))
        def f2(): return  tf.zeros(batch_size_ctrl, dtype=self.tf_prec)
        cc_energy = tf.cond(AddEcc, f1, f2)
        return  cc_energy,dipole, final_charge, dipole_wb

    def loss_op_dipole(self,energy,energy_grads,dipole,charge,Dlabels,Qlabels, natom):
        ...
        charge_diff = tf.multiply(tf.subtract(charge, Qlabels), tf.reshape(natom*maxatom,[-1,1]))
        dipole_loss = tf.nn.l2_loss(dipole_diff)*self.DipoleScalar
        charge_loss = tf.nn.l2_loss(charge_diff)*self.DipoleScalar
        loss = tf.identity(charge_loss)
        tf.add_to_collection('losses', loss)
        total_loss=tf.add_n(tf.get_collection('losses'),name='total_loss')
        return total_loss,loss,dipole_loss,charge_loss
    ...
    def train_step_dipole(self, step):
        ...
        for ministep in range (0, int(Ncase_train/self.batch_size)):
            ...
            t             = time.time()
            dump_2, total_loss_value, loss_value, dipole_loss,charge_loss,lr_dipole \
                = self.sess.run([self.train_op_dipole, self.total_loss_dipole, self.loss_dipole, self.dipole_loss_dipole, self.charge_loss_dipole,self.lr_dipole],\
                     feed_dict=self.fill_feed_dict(batch_data))
            print_loss        += loss_value
            print_charge_loss += charge_loss

            print_dipole_loss += dipole_loss
            if (ministep%print_per_mini == 0 and ministep!=0):
                print ("time:", (time.time() - time_print_mini)/print_per_mini, "loss_value: ",  print_loss/print_per_mini, " charge_loss:", print_charge_loss/print_per_mini)
                print_loss        = 0.0
                print_charge_loss = 0.0
                print_dipole_loss = 0.0
        ...
        self.print_training_DandQ(step, train_charge_loss, num_of_mols, duration,lr_dipole)
        return train_charge_loss/num_of_mols
    ...
    def test_dipole(self, step):
        ...
        for ministep in range (0, int(Ncase_test/self.batch_size)):
            ...
            actual_mols  = self.batch_size
            t            = time.time()
            total_loss_value, loss_value, dipole_loss, charge_loss =\
                 self.sess.run([self.total_loss_dipole, self.loss_dipole,\
                 self.dipole_loss_dipole, self.charge_loss_dipole], feed_dict=self.fill_feed_dict(batch_data))
            ...
            num_of_mols      += actual_mols
        #print ("testing...")
        self.print_training_DandQ(step,  test_charge_loss, num_of_mols, duration,0,False)
    ...
    def loss_op_EandG(self, energy, energy_grads, Elabels, grads, natom):
        maxatom=tf.cast(tf.shape(energy_grads)[2], self.tf_prec)
    ...
    def train_step_EandG(self, step):
        ...
        for ministep in range (0, int(Ncase_train/self.batch_size)):
            ...
            actual_mols= self.batch_size
            t          = time.time()
            dump_2, total_loss_value, loss_value, energy_loss, grads_loss,  Etotal,lr_ef =\
                 self.sess.run([self.train_op_EandG, self.total_loss_EandG, self.loss_EandG,\
                     self.energy_loss_EandG, self.grads_loss_EandG, self.Etotal,self.lr_ef], feed_dict=self.fill_feed_dict(batch_data))
            print_loss           += loss_value
            print_energy_loss    += energy_loss
            print_grads_loss     += grads_loss

            if (ministep%print_per_mini == 0 and ministep!=0):
                print ("time:", (time.time() - time_print_mini)/print_per_mini ,  " loss_value: ",  print_loss/print_per_mini, " energy_loss:", print_energy_loss/print_per_mini, " grads_loss:", print_grads_loss/print_per_mini)
                print_loss        = 0.0
                print_energy_loss = 0.0
            ...
            num_of_mols       += actual_mols
        duration           = time.time() - start_time
        self.print_training_EandG(step, train_energy_loss, train_grads_loss, num_of_mols, duration,lr_ef)
    ...
    def test_EandG(self, step):
        ...
        for ministep in range (0, int(Ncase_test/self.batch_size)):
            ...
            actual_mols  = self.batch_size
            t            = time.time()
            total_loss_value, loss_value, energy_loss,grads_loss =\
                 self.sess.run([self.total_loss_EandG, self.loss_EandG, self.energy_loss_EandG, self.grads_loss_EandG], feed_dict=self.fill_feed_dict(batch_data))
            test_loss         = test_loss + loss_value
            ...
            num_of_mols      += actual_mols
        print ("testing...")
        self.print_training_EandG(step, test_energy_loss, test_grads_loss, num_of_mols, duration,0,False)
        return  test_energy_loss,test_grads_loss 

    def print_training_EandG(self, step, energy_loss, grads_loss, Ncase, duration,lr=0, Train=True):
        if Train:
            self.recorder.write("step: %7d L_rate: %.6f duration: %.5f  Train Loss:  E: %.6f  F: %.6f "%(step,lr, duration, (float(energy_loss)/(Ncase)), (float(grads_loss)/(Ncase))))
            self.recorder.flush()
        else:
            self.recorder.write("Test Loss: E: %.6f F: %.6f \n" %(float(energy_loss)/(Ncase), float(grads_loss)/(Ncase)))
            self.recorder.flush()
        return
    ...
    def evaluate(self, batch_data):
        ...
        Etotal, Ebp, Ebp_atom, Ecc, Evdw, mol_dipole, atom_charge, gradient =\
             self.sess.run([self.Etotal, self.Ebp, self.Ebp_atom, self.Ecc, self.Evdw, self.dipole, self.charge, self.gradient], feed_dict=feed_dict)
