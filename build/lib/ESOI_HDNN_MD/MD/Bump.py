...
class BumpHolder():
    ...
    def Prepare(self):
        ...
        with self.Bumpgraph.as_default():
            ...
            self.Bumpenergy=tf.reduce_sum(-1*self.h*tf.exp(-0.5*self.frame_diff/tf.square(self.w)))
            self.Bumpforces=tf.gradients(self.Bumpenergy,self.xyzs_pl)
            self.Bumpforcet=tf.gradients(self.Bumpenergy,self.xyzt_pl)
            #self.Bumpenergy=tf.
    ...
    def Bump(self,xyzs,xyzt,dis_pl):
        BE,BFs,BFt=self.Bumpsess.run([self.Bumpenergy,self.Bumpforces,self.Bumpforcet],{self.xyzs_pl:xyzs,self.xyzt_pl:xyzt,self.dis_pl:dis_pl})
        return BE,BFs,BFt
...
dis_pl=np.array([[[0],[0]],[[1],[1]]]*100)
bump=BumpHolder(2,2,1,1)
bump.Prepare()
BE,BFs,BFt=bump.Bump(xyzs,xyzt,dis_pl)
print (BE,BFs,BFt)
